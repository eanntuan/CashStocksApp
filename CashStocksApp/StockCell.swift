...
class StockCell: UITableViewCell {
  ...
  @IBOutlet weak var stockPercentageChange: UILabel!
  ...
  func configure(_ index: IndexPath) {
    if let stock = stock {
      ...
      let change = "+50%" // stock.percentage from backend
      stockPercentageChange.text = "\(change)"
      percentageWrapper.backgroundColor = change.first == "+"
